...
export default function Home() {
  ...
  const scrollRail = (index: number, direction: number) => {
    ...
    const nextPosition = Math.max(0, Math.min(rail.scrollLeft + direction * cardStep * visibleCards, maxPosition));
    rail.scrollTo({ left: nextPosition, behavior: "smooth" });
    // Some browsers ignore smooth scrolling while a scroll-snap rail is settling.
    // Set the final position once more so every click always advances the gallery.
    window.setTimeout(() => { rail.scrollLeft = nextPosition; }, 420);
  };
