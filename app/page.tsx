...
export default function Home() {
  ...
  const scrollRail = (index: number, direction: number) => {
    ...
    const maxPosition = Math.max(0, rail.scrollWidth - viewport.clientWidth);
  ...
  return (
    <main>
      ...
      <section className="work section frame" id="work">
        ...
        <div className="project-groups">{projectGroups.map((group, groupIndex) => { const hasControls = group.controls; const isArt = Number(group.id) >= 3; return <Fragment key={group.id}>{group.id === "03" && <div className="art-collection-heading"><span>03 / ART DESIGN</span><h3>美术类设计</h3><p>角色设计、海报设计与三维建模作品。</p></div>}<section className={`project-group ${isArt ? "art-showcase" : ""}`}><div className="project-group-heading"><span>{group.id} / {group.en}</span><h3>{group.title}</h3><p>{group.intro}</p><small>{hasControls ? "点击左右按钮预览作品" : "横向滑动浏览　→"}</small></div><div className="project-rail-wrap"><div className="project-rail-viewport"><div className="project-rail" style={{ transform: `translate3d(-${railOffsets[groupIndex] ?? 0}px, 0, 0)` }} ref={(element) => { rails.current[groupIndex] = element; }}>{group.projects.map(([id, title, note, image, tone]) => <article className={`project-card ${tone}`} key={`${group.id}-${id}`}><div className="project-image"><img src={image} alt={title} /></div><div className="project-detail"><span>{group.id}.{id} / {group.en}</span><h3>{title}</h3>{!isArt && <a href="#contact">查看项目 <b>↗</b></a>}</div></article>)}</div></div>{hasControls && <button className="gallery-side-button gallery-side-button--prev" type="button" onClick={() => scrollRail(groupIndex, 1)} aria-label={`向左滑动查看${group.title}作品`}><span>←</span></button>}{hasControls && <button className="gallery-side-button gallery-side-button--next" type="button" onClick={() => scrollRail(groupIndex, -1)} aria-label={`向右滑动查看${group.title}作品`}><span>→</span></button>}</div></section></Fragment>; })}</div>
      </section>

      <section className="strength section" id="strength"><div className="frame"><div className="section-number">04 <span>— MY STRENGTHS</span></div><MagicTitle first="我的优势" second="" /><div className="strength-grid">{strengths.map(([number, title, body]) => <article key={number}><span>{number}</span><i>✦</i><h3>{title}</h3><p>{body}</p></article>)}</div></div></section>
