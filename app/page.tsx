  ...
  return (
    <main>
      ...
      <section className="work section frame" id="work">
        ...
        <div className="project-groups">{projectGroups.map((group, groupIndex) => { const hasControls = group.controls; const isArt = Number(group.id) >= 3; return <Fragment key={group.id}>{group.id === "03" && <div className="art-collection-heading"><span>03 / ART DESIGN</span><h3>美术类设计</h3><p>角色设计、海报设计与三维建模作品。</p></div>}<section className={`project-group ${isArt ? "art-showcase" : ""}`}><div className="project-group-heading"><span>{group.id} / {group.en}</span><h3>{group.title}</h3><p>{group.intro}</p><small>{hasControls ? "点击左右按钮预览作品" : "横向滑动浏览　→"}</small></div><div className="project-rail-wrap">{hasControls && <div className="gallery-controls" role="group" aria-label={`${group.title}作品预览`}><button className="gallery-prev" type="button" onClick={() => scrollRail(groupIndex, -1)} aria-label={`查看${group.title}上一个项目`}><span>←</span></button><button className="gallery-next" type="button" onClick={() => scrollRail(groupIndex, 1)} aria-label={`查看${group.title}下一个项目`}><span>→</span></button></div>}<div className="project-rail-viewport"><div className="project-rail" style={{ transform: `translate3d(-${railOffsets[groupIndex] ?? 0}px, 0, 0)` }} ref={(element) => { rails.current[groupIndex] = element; }}>{group.projects.map(([id, title, note, image, tone]) => <article className={`project-card ${tone}`} key={`${group.id}-${id}`}><div className="project-image"><img src={image} alt={title} /></div><div className="project-detail"><span>{group.id}.{id} / {group.en}</span><h3>{title}</h3>{!isArt && <a href="#contact">查看项目 <b>↗</b></a>}</div></article>)}</div></div></div></section></Fragment>; })}</div>
      </section>
